...
public struct NotificationsRequestsListView: View {
  ...
  private func fetchRequests(_ client: FediverseClient) async {
    do {
  ...
  private func acceptRequest(_ client: FediverseClient, _ request: NotificationsRequest) async {
    _ = try? await client.post(endpoint: Notifications.acceptRequests(ids: [request.id]))
    await fetchRequests(client)
  }

  private func dismissRequest(_ client: FediverseClient, _ request: NotificationsRequest) async {
    _ = try? await client.post(endpoint: Notifications.dismissRequests(ids: [request.id]))
    await fetchRequests(client)
  }

  private func acceptSelectedRequests(_ client: FediverseClient) async {
  ...
  private func rejectSelectedRequests(_ client: FediverseClient) async {
    isProcessingRequests = true
    _ = try? await client.post(
