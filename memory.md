...
- **Bug Fix:** The masonry "Gallery" view layout (used in profiles and specific timeline configurations) was completely broken (leaving massive vertical gaps and single columns). This was caused by an unintended layout regression in `GalleryStatusesListView` when moving properties out of a view block. Restored `GalleryStatusesListView` to its correct state (`6a51dc50b1`), maintaining the correct `LazyVStack` and `HStack` staggered layout logic.
