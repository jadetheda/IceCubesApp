...
- **2026-08-08T16:59:00Z**: Fixed the SwiftUI VideoPlayer bug where initialized players that start as nil do not render when updated by adding .id(viewModel.player != nil ? "player-loaded" : "player-nil"). Disabled UIFileSharingEnabled and LSSupportsOpeningDocumentsInPlace to prevent the app from cluttering the iOS Files app. Fixed Settings Logs button displaying 'No logs found' incorrectly before any logs are created.

- **2026-08-08T17:11:00Z**: Replaced SwiftUI VideoPlayer '.id()' workaround with proper state-retaining AVPlayer architecture. Reusing the AVPlayer instance and invalidating old AVPlayerItem observers prevents silent loading failures. Also added 'AVURLAssetOverrideMIMETypeKey' alongside 'AVURLAssetOutOfBandMIMETypeKey' to support iOS 17+ extensionless video hints.
