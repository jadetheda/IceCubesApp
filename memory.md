...
- **2026-08-04T08:18:00Z**: Fully ripped out all chunking logic (including `GalleryChunk`, `chunkItems`, `GallerySegment`, `makeSegments`, `makeGridChunk`) from `GalleryStatusesListView.swift`. The code has been completely reverted to the single continuous `HStack` masonry grid. `TimelineGap` items are safely routed natively into column 0 as documented, allowing us to maintain the `maxHeight: .infinity, alignment: .top` fix without the layout fragmentation of chunks.
