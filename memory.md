...
- 2026-08-23T14:25:00-07:00: **Refactored Gallery Mode to eliminate `AccountMediaFetcher` ViewModel**
  - Adhered strictly to `CLAUDE.md` rules against custom ViewModels by removing the fragmented `AccountMediaFetcher` implementation used by `AccountDetailMediaGridView`.
...
- **2026-09-02**: Carefully merged `upstream/main` into the `gallery_mode_refactor` branch, resolving package dependency conflicts and preserving our custom Bluesky theme variants.
- **2026-09-02**: Fixed Exit Code 65 compilation failures caused by the upstream merge. Corrected malformed `deinit` brackets in `MediaUIAttachmentVideoView.swift` and removed an orphaned `.custom` enum switch case in `Theme.swift`.
  - **Fix (DisplaySettingsView):** Upstream added a new `.inter` (Inter - Bluesky) font to `Theme.FontState`. Because our local resolution of `DisplaySettingsView.swift` bypassed it to keep our custom fonts, it left the `switch newValue` in `DisplaySettingsView` without an `.inter` case. Swift enums require exhaustive switches, resulting in an Exit Code 65 compile failure. Added `.inter` back to the switch block and the getter block.
  - **Fix (Timeline Package):** Upstream updated the Nuke dependency in `DesignSystem` to version `13.2.0`, but the `Timeline` package was still explicitly requesting exact version `12.8.0`, causing an Exit Code 74 package resolution conflict. Updated `Timeline` to use Nuke `13.2.0`.
  - **Fix (ColorSet.swift):** Upstream introduced their own `BlueskyDark` and `BlueskyLight` color sets, which caused a merge conflict with our existing `Bluesky` themes resulting in duplicated struct definitions. The duplicate structs lacked our required protocol properties (e.g., `isLikeAction`, `bookmarkColor`), resulting in Exit Code 65 `invalid redeclaration` and protocol conformance failures. Safely removed the duplicate upstream structs and retained our fully-featured implementations.
- **2026-09-04 20:58 UTC**: Refactored Gallery Mode to function as a universal filter mode integrated deeply into `StatusesListView`. Ripped out `GalleryStatusesListView` as a standalone structural component and embedded its staggered masonry math directly into `StatusesListView`. This prevents `TimelineListView` and `AccountStatusesListView` from constantly mounting/unmounting their `List` containers when the filter is toggled, thus resolving the scroll-position loss issues when transitioning in and out of Gallery Mode.
- **2026-09-04 21:21 UTC**: Fixed an Exit Code 65 build failure in `StatusesListView.swift`. A trailing `}` brace was erroneously left at the top level after integrating the gallery math, which broke the `StatusKit` package resolution. Removed the extraneous brace to restore compilation integrity.

- 2026-09-05 (UTC): **Fixed Exit Code 65 Circular Dependency**: Diagnosed compile failure in `StatusesListView` caused by `import Timeline` which created a circular dependency (`StatusKit` -> `Timeline` -> `StatusKit`). Resolved by removing `TimelineContentFilter` from `StatusesListView` and instead passing `isForceGalleryMode` explicitly from the caller in `TimelineListView.swift`. Restored compiler stability and fixed layout rendering nesting.
- 2026-09-05 (UTC): **Fixed Gallery Scrolling and Boosts Tab Profile Issues**: The recent refactor that merged Gallery mode deeply into `StatusesListView` broke timeline scrolling, as `TimelineListView` wrapped the new gallery layout inside a standard `List`, treating thousands of images as a single monolithic row. Restored `ScrollView` for gallery mode in `TimelineListView` and `AccountDetailMediaGridView`. Additionally, resolved user complaint that BoostsTab (and other profile tabs) no longer responded to the Gallery Mode toggle. Introduced `supportsGalleryMode` to `AnyStatusesListView`, allowing Bookmarks, Favorites, Boosts, and Media tabs to render as gallery grids when the global filter is active, while still honoring previous restrictions preventing Gallery mode from hijacking the main Posts and Replies tabs.
- **2026-09-04 22:42 UTC**: Fixed critical Gallery Mode scroll-position loss and prepending jump issues.
  - Users reported that toggling Gallery Mode lost scroll position, and that newly fetched posts (prepended at the top) were pushing the visible viewport down instead of loading silently above the scroll anchor.
  - Reverted `TimelineListView` and `AccountDetailMediaGridView` to use `List` exclusively for both standard and Gallery modes, as `ScrollView` lacks native iOS 15/16 scroll-anchoring when items are prepended.
  - Implemented deterministic hash-based chunking (`status.id.hashValue % 18 == 0`) in `StatusesListView.swift`. This mathematically guarantees that prepending new items never alters the chunk boundaries or internal layout assignments of older items. Because the older chunks remain completely stable and their IDs (using `items.last?.id`) are perfectly preserved, `List` can natively maintain the scroll anchor when new items are added at the top.
- **2026-09-04 23:00 UTC**: Reverted Gallery Mode to `ScrollView` + `LazyVStack` architecture to restore cache scroll anchoring.
  - The previous switch to `List` in Gallery Mode natively broke initial load cache scroll restoration (`TimelineViewModel` sets a `scrollToId` on load) because `ScrollViewReader` within a `List` can only target immediate row IDs, preventing it from finding individual `Status.id`s nested deep within the masonry chunks.
  - By restoring `ScrollView { LazyVStack { ... } }`, `ScrollViewReader` regains its ability to deeply traverse the layout tree and find the hidden anchor `Color.clear.id(status.id)`, completely fixing the issue where loading the app forces the viewport to the top of the newest fetched posts instead of resting on the cached posts.
- **2026-09-05 06:19 UTC**: Fixed compilation parameter ordering bug in `StatusesTab.swift` where `showFilterWarning` was placed after `supportsGalleryMode` causing Exit Code 65. Codemagic build has now passed successfully.
