...
- **2026-08-05T01:27:00Z**: Fixed empty/collapsed image carousels in Timeline. Restored explicit width and height constraints in `StatusRowMediaPreviewView` for non-standalone images. A previous automated refactor removed the explicit width modifiers, causing `LazyResizableImage` elements with `.aspectRatio(contentMode: .fill)` inside a `ScrollView(.horizontal)` to collapse to a width of 0 because horizontal ScrollViews do not provide horizontal bounding constraints.
- **2026-08-05T02:00:00Z**: Corrected inner `frame` modifier inside `MediaPreview` `.image` case in `StatusRowMediaPreviewView` to ensure `width` and `height` are explicitly constrained when `!isStandalone`, preventing `LazyResizableImage` from collapsing to width 0 during loading in the carousel.
- **2026-08-05T02:14:00Z**: Added optional "Hide Pinned" / "Show Pinned" button to the Timeline menu to collapse pinned filter pills. Added options in Display Settings to toggle the presence of this button and to optionally hide the icons (symbols) inside the pinned pills to save horizontal space.
- **2026-08-05T02:22:00Z**: Relocated "Hide/Show Pinned" button to the bottom of the Timeline menu as requested, and moved its visibility toggle to the Content Settings to clearly separate it from the "Hide Pinned Items Symbol" feature in Display Settings.
- **2026-08-05T02:28:00Z**: Relocated the visibility toggle for the "Hide Pinned" Timeline menu button back to the Display Settings page, adjacent to the Pinned Items Symbol toggle.
